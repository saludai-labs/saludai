...
if [ "${PATIENT_COUNT}" -ge 190 ]; then
  echo "==> Seed verification PASSED (${PATIENT_COUNT} >= 190 patients)"
else
  echo "==> ERROR: Seed verification FAILED (${PATIENT_COUNT} < 190 patients)"
  exit 1
fi

# Verify all 10 resource types
for RESOURCE_TYPE in Condition Observation MedicationRequest Encounter Procedure AllergyIntolerance Immunization DiagnosticReport CarePlan; do
  COUNT=$(curl -s "${FHIR_SERVER_URL}/${RESOURCE_TYPE}?_summary=count" \
    | grep -o '"total" *: *[0-9]*' \
    | grep -o '[0-9]*')
