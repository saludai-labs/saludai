...
**An open-source FHIR reasoning agent with multi-LLM benchmarks and locale-aware terminology** — every reasoning step auditable via Langfuse, designed for public health systems.

[![PyPI](https://img.shields.io/pypi/v/saludai)](https://pypi.org/project/saludai/)
...
**Prerequisites:** [Python 3.12+](https://python.org), [Docker](https://docs.docker.com/get-docker/)
...
# Run the agent
saludai query "¿Cuántos pacientes tienen diabetes tipo 2?"
```

### Development Setup

```bash
# Clone and install with UV (for contributors)
git clone https://github.com/saludai-labs/saludai.git
cd saludai
uv sync
...
```bash
saludai serve
# POST http://localhost:8000/query {"question": "Pacientes con hipertensión en Córdoba"}
```
...
SaludAI is open source, auditable, and self-hostable. Country-specific terminology, prompts, and FHIR metadata are packaged as locale packs — swap the locale, keep the same agent:

- **Locale packs:** bundles of terminology codes, system prompts, tool descriptions, and FHIR metadata. Argentina ships built-in; add your country by implementing a locale pack
- **4 terminology systems:** SNOMED CT, CIE-10, LOINC, ATC — with fuzzy matching via rapidfuzz
- **FHIR profile awareness:** locale packs reference national FHIR profiles (e.g., openRSD for Argentina)
- **Localized prompts:** the agent reasons in the language of the data
