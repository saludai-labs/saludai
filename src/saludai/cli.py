...
def main() -> None:
    ...
    if not args or args[0] in ("--help", "-h"):
        ...
        print(
            "  SALUDAI_FHIR_SERVER_URL    FHIR R4 server URL (default: http://localhost:8890/fhir)"
        )
...
async def _execute_query(query: str) -> None:
    ...
    has_key = agent_cfg.llm_api_key or os.environ.get(
        {
            "anthropic": "ANTHROPIC_API_KEY",
            "openai": "OPENAI_API_KEY",
        }.get(agent_cfg.llm_provider, ""),
    )
    if not has_key:
        print(
            "Error: No API key found. Set SALUDAI_LLM_API_KEY, "
            "or ANTHROPIC_API_KEY / OPENAI_API_KEY for the chosen provider.",
            file=sys.stderr,
        )
        sys.exit(1)
