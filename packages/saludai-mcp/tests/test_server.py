...
from saludai_mcp.server import _lifespan, mcp
...
class TestMCPConfig:
    ...
    def test_defaults(self) -> None:
        cfg = MCPConfig()
        assert cfg.fhir_server_url == "http://localhost:8890/fhir"
...
class TestLifespan:
    """Tests for the MCP server lifespan (startup/shutdown)."""
    ...
    async def test_lifespan_initialises_and_cleans_up(self) -> None:
        ...
        with (
            patch("saludai_mcp.server.FHIRClient", return_value=mock_fhir),
            patch("saludai_mcp.server.load_locale_pack", return_value=None),
        ):
            async with _lifespan(mcp):
                assert srv._fhir_client is mock_fhir
                assert srv._terminology_resolver is not None

        assert srv._fhir_client is None
        assert srv._terminology_resolver is None
        mock_fhir.close.assert_called_once()

    @pytest.mark.anyio()
    async def test_lifespan_handles_locale_error(self) -> None:
        import saludai_mcp.server as srv
        ...
        with (
            patch("saludai_mcp.server.FHIRClient", return_value=mock_fhir),
            patch("saludai_mcp.server.load_locale_pack", side_effect=Exception("bad")),
        ):
            async with _lifespan(mcp):
                assert srv._terminology_resolver is not None
