...
class TestToolDefinitions:
    ...
    def test_resolve_terminology_system_enum(self) -> None:
        ...
        assert set(system_prop["enum"]) == {"snomed_ct", "cie_10", "loinc", "atc"}
...
def _make_mock_fhir_client(bundle: Any = None) -> MagicMock:
    ...
    client = MagicMock()
    default_bundle = bundle or _make_empty_bundle()
    client.search = AsyncMock(return_value=default_bundle)
    client.search_all = AsyncMock(return_value=default_bundle)
    return client
...
class TestExecuteSearchFhir:
    ...
    async def test_empty_search(self) -> None:
        ...
        # Default _count=200 is injected, uses search_all for pagination
        client.search_all.assert_called_once_with("Patient", {"_count": "200"})
    ...
    async def test_search_with_params(self) -> None:
        ...
        # _count=200 injected alongside existing params, uses search_all
    ...
    async def test_default_count_not_injected_when_count_present(self) -> None:
        ...
        # Custom _count preserved, still uses search_all for pagination
        client.search_all.assert_called_once_with("Patient", {"_count": "50"})
    ...
    async def test_default_count_not_injected_when_summary_present(self) -> None:
        ...
        await execute_search_fhir(client, {"resource_type": "Patient", "params": params})
        # _summary queries use search (no pagination needed)
...
class TestFormatBundleSummary:
    ...
    def test_small_bundle_lists_all_resources(self) -> None:
        """≤30 resources: list every resource individually."""
        bundle = _make_bundle_with_patients(5)
        result = format_bundle_summary(bundle)
        # All 5 patients listed individually
        for i in range(5):
            assert f"patient-{i}" in result
        assert "Sample" not in result
        assert "entries" not in result.lower() or "execute_code" not in result

    def test_large_bundle_shows_smart_summary(self) -> None:
        """&gt;30 resources: smart summary with stats + sample of 10."""
        entries = []
        for i in range(50):
            entries.append(
                {
                    "resource": {
                        ...
                        "vaccineCode": {
                            "coding": [
                                {
                                    "code": "10" if i < 30 else "19",
                                    "display": "IPV" if i < 30 else "BCG",
                                }
                            ],
                        },
                        "patient": {"reference": f"Patient/{i % 20}"},
        ...
        result = format_bundle_summary(bundle)
        assert "Unique patients: 20" in result
        assert "IPV" in result
        assert "BCG" in result
        assert "Sample (10 of 50)" in result
        assert "execute_code" in result
        # Should NOT list all 50
        assert "imm-49" not in result

    def test_large_bundle_date_range(self) -> None:
        """Smart summary includes date range."""
        entries = []
        for i in range(35):
            entries.append(
                {
                    "resource": {
                        ...
                        "id": f"c-{i}",
                        "code": {
                            "coding": [{"code": "44054006", "display": "DM2"}],
                        },
                        "subject": {"reference": f"Patient/{i}"},
                        "onsetDateTime": f"202{3 + i % 3}-01-15",
                        "clinicalStatus": {"text": "active"},
                    }
                }
            )
...
class TestExecuteCode:
    ...
    def test_extra_globals_entries(self) -> None:
        """execute_code can access injected entries from scratchpad."""
        fhir_entries = [
            {"resourceType": "Patient", "id": "1", "gender": "male"},
            {"resourceType": "Patient", "id": "2", "gender": "female"},
            {"resourceType": "Patient", "id": "3", "gender": "male"},
        ]
        code = "print(len(entries))"
        result = execute_code({"code": code}, extra_globals={"entries": fhir_entries})
        assert "3" in result

    def test_entries_count_unique(self) -> None:
        """Common pattern: count unique patients from entries."""
        fhir_entries = [
    ...
    def test_entries_empty_by_default(self) -> None:
        """Without extra_globals, entries is not available."""
        result = execute_code({"code": "print(entries)"})
        assert "Error" in result

    def test_store_persists_across_calls(self) -> None:
        """store dict persists across multiple execute_code calls."""
        store: dict[str, Any] = {}
        # First call: save data
        execute_code(
            {"code": "store['patients'] = {'P/1', 'P/2', 'P/3'}"},
            extra_globals={"entries": [], "store": store},
        )
        assert "patients" in store
        assert len(store["patients"]) == 3

        # Second call: read saved data (with different entries)
        result = execute_code(
            {"code": "print(len(store['patients']))"},
            extra_globals={"entries": [{"id": "other"}], "store": store},
        )
        assert "3" in result

    def test_store_cross_reference_pattern(self) -> None:
        """store enables the cross-reference pattern: save set A, search B, intersect."""
        store: dict[str, Any] = {}
        entries_a = [
            {"resourceType": "Condition", "subject": {"reference": "Patient/1"}},
            {"resourceType": "Condition", "subject": {"reference": "Patient/2"}},
            {"resourceType": "Condition", "subject": {"reference": "Patient/3"}},
        ]
        # Step 1: save patients from first search
        execute_code(
            {"code": "store['set_a'] = set(e['subject']['reference'] for e in entries)"},
            extra_globals={"entries": entries_a, "store": store},
        )

        # Step 2: new search overwrites entries
        entries_b = [
            {"resourceType": "Observation", "subject": {"reference": "Patient/2"}},
            {"resourceType": "Observation", "subject": {"reference": "Patient/4"}},
        ]
        result = execute_code(
            {
                "code": (
                    "set_b = set(e['subject']['reference'] for e in entries)\n"
                    "both = store['set_a'] & set_b\n"
                    "print(len(both))"
                ),
            },
            extra_globals={"entries": entries_b, "store": store},
        )
        assert "1" in result  # Only Patient/2 in both
...
class TestToolRegistry:
    ...
    def test_definitions_without_resolver(self) -> None:
        ...
        defs = registry.definitions()
        names = {d["name"] for d in defs}
        assert names == {"search_fhir", "count_fhir", "get_resource", "execute_code"}
    ...
    async def test_execute_search_fhir(self) -> None:
        ...
        client.search_all = AsyncMock(return_value=bundle)
        registry = ToolRegistry(fhir_client=client)
        tc = ToolCall(
            id="tc_1",
            name="search_fhir",
    ...
    @pytest.mark.asyncio
    async def test_store_persists_via_registry(self) -> None:
        """store dict in ToolRegistry persists across multiple execute_code calls."""
        client = MagicMock()
        registry = ToolRegistry(fhir_client=client)
        # First call: save to store
        tc1 = ToolCall(
            id="tc_1",
            name="execute_code",
            arguments={"code": "store['x'] = 42; print('saved')"},
        )
        r1 = await registry.execute(tc1)
        assert "saved" in r1.content
        # Second call: read from store
        tc2 = ToolCall(
            id="tc_2",
            name="execute_code",
            arguments={"code": "print(store['x'])"},
        )
        r2 = await registry.execute(tc2)
    ...
    async def test_execute_tool_error_raises(self) -> None:
        ...
        client.search_all = AsyncMock(side_effect=Exception("connection failed"))
        registry = ToolRegistry(fhir_client=client)
        tc = ToolCall(
            id="tc_1",
            name="search_fhir",
