...
class TestAgentLoopDefaultConfig:
    ...
    def test_default_max_iterations_is_12(self) -> None:
        config = AgentConfig()
        assert config.agent_max_iterations == 12
...
class TestAgentLoopActionSpaceReduction:
    """Planner-driven tool filtering (Action Space Reduction)."""

    @pytest.mark.asyncio
    async def test_count_strategy_hides_search_fhir(self) -> None:
        """When planner selects count_simple, only count_fhir is visible."""
        ...
        await loop.run("Cuantos pacientes hay?")

        # Verify only count_fhir was passed to the executor
        tools = executor_llm.calls[0]["tools"]
        tool_names = {t["name"] for t in tools}
        assert tool_names == {"count_fhir"}
        assert "search_fhir" not in tool_names
        assert "get_resource" not in tool_names

    @pytest.mark.asyncio
    async def test_multi_search_shows_all_tools(self) -> None:
        """When planner selects multi_search, all tools remain visible."""
        from saludai_core.locales import load_locale_pack
        ...
        await loop.run("Correlacion entre diabetes y medicaciones")

        # All tools should be present (multi_search has empty allowed_tools)
        tools = executor_llm.calls[0]["tools"]
        tool_names = {t["name"] for t in tools}
        assert "search_fhir" in tool_names
        assert "count_fhir" in tool_names
        assert "resolve_terminology" in tool_names
    ...
    async def test_planner_disabled_shows_all_tools(self) -> None:
        ...
        fhir_client = _make_fhir_client()
        resolver = _make_terminology_resolver()
        config = AgentConfig(planner_enabled=False)
        ...
        tools = executor_llm.calls[0]["tools"]
        tool_names = {t["name"] for t in tools}
        assert len(tool_names) == 5  # all 5 tools
