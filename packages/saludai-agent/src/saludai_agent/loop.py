...
class AgentLoop:
    ...
    async def run(self, query: str) -> AgentResult:
        ...
        self._tracer.start_trace(
            ...
            metadata={
                ...
                "planner_enabled": self._config.planner_enabled,
            },
        )

        # Planning phase (ADR-009)
        system_prompt = self._system_prompt
        allowed_tools: frozenset[str] | None = None
        if self._config.planner_enabled and self._locale_pack is not None:
            query_plan = await self._run_planner(query)
            plan_block = format_plan_for_prompt(query_plan)
            system_prompt = f"{self._system_prompt}\n\n{plan_block}"

            # Action Space Reduction: resolve which tools the executor may see
            allowed_tools = resolve_tool_set(
                query_plan.strategy,
                self._locale_pack.query_patterns,
            )
            logger.info(
    ...
    async def _run_planner(self, query: str) -> QueryPlan:
        """Run the planning phase to classify the query and select a strategy.

        Uses the planner LLM (which may be a different, cheaper model) to
        analyze the question against the FHIR knowledge from the locale pack.
