...
Usage:
    ...
    uv run python -m benchmarks.run_eval --resume benchmarks/results/progress_20260311_120000.jsonl
"""
...
from benchmarks.results import load_completed_ids, print_summary, write_results_json
...
from saludai_agent.tracing import CompositeTracer, RecordingTracer, create_tracer
...
async def _run(args: argparse.Namespace) -> None:
    ...
    if args.resume:
        ...
        if completed:
            ...
            questions = [q for q in questions if q.id not in completed]
            skipped = before - len(questions)
            print(f"Resuming: {len(completed)} already completed, {skipped} skipped")
    else:
        # New run: create fresh progress file
    ...
    locale_pack = load_locale_pack(agent_config.locale)
    resolver = TerminologyResolver(locale_pack=locale_pack)

    # Tracing: Langfuse/NoOp + RecordingTracer via CompositeTracer
    base_tracer = create_tracer(agent_config)
