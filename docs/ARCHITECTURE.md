...
SaludAI es un agente FHIR inteligente diseñado como paquetes independientes que comparten infraestructura base. El diseño prioriza:
...
4. **Extensibilidad** — La arquitectura de paquetes y locale packs permite crecer sin reescrituras.
...
| Componente | Lenguaje alternativo | Justificación |
|-----------|---------------------|---------------|
| Terminology server de alta performance | **Rust** (o usar Snowstorm en Java) | Fuzzy matching + trie traversal sobre millones de conceptos SNOMED es CPU-bound |
| CLI distribuible sin runtime | **Rust** (vía PyO3) o **Go** | Python requiere runtime instalado; Rust/Go compilan a binarios estáticos |
| Frontend / dashboard | **TypeScript** (React/Next.js) | Dashboards interactivos necesitan UI rica |
| MCP server (alternativa) | **TypeScript** | Si la comunidad MCP migra mayoritariamente a TS |
...
### 4.2 Hybrid Query Planner (ADR-009)

```
User Question
      |
      v
[PLANNER] -- 1 LLM call, sin tools, structured JSON output
  |  Inputs: pregunta + catalogo de patrones FHIR + grafo de referencias
  |  Output: QueryPlan {question_type, strategy, terms_to_resolve, reasoning}
  |
  v  QueryPlan inyectado en system prompt
[EXECUTOR] -- ReAct loop existente, plan-aware
  |  Tools: resolve_terminology, search_fhir, count_fhir, get_resource, execute_code
  |  Si plan falla -> fallback a ReAct libre
  |
  v
AgentResult
```

El conocimiento FHIR se modela como datos estructurados (no texto en el prompt):

- **Grafo de referencias** (`ResourceRelationship`): aristas entre resource types con search params. Permite derivar `_include`, `_revinclude`, y `_has` patterns automaticamente.
- **Catalogo de patrones** (`QueryPattern`): templates de query FHIR validados (count_simple, count_with_condition, search_aggregate, multi_search, etc.).
- **Ambos viven en el locale pack** — extensibles por pais/region.

El LLM maneja lo fuzzy (clasificacion NLP, extraccion de terminos). El catalogo maneja lo preciso (sintaxis FHIR valida, patrones probados).

El diseno escala gradualmente: catalogo -> RAG, grafo -> graph DB, rapidfuzz -> embeddings. Ver `docs/knowledge/fhir-query-planner-design.md` para criterios de escalamiento.
...
| 007 | Sistema de locale packs para extensibilidad por pais |
| 008 | FHIR Awareness en locale packs (Level 1 metadata) |
| 009 | Hybrid Query Planner — Plan-and-Execute con FHIR knowledge graph |
